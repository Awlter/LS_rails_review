...
describe Category do
  ...
  describe "#recent_videos" do
    let(:show) { create(:category) }
    it "returns an array of videos ordered from the lastest created" do
      video1 = create(:video, category: show)
      video2 = create(:video, created_at: 1.day.since, category: show)
      video3 = create(:video, created_at: 1.day.ago, category: show)
    ...
    it "returns an array containing 6 videos if there are more than 6 found for that category" do
      8.times { create(:video, category: show) }
